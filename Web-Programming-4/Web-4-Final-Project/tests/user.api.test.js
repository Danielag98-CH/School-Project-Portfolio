...
describe('Users API Tests', () => {
  ...
  describe('GET /users/:id', () => {
    ...
    it('should return 404 if user not found', async () => {
      const response = await agent.get('/users/999').set("Authorization", `Bearer ${adminToken}`);
  ...
  describe('POST /users', () => {
    ...
    it('should return 201 and the new user id (for admins)', async () => {
      const response = await agent.post('/users')
                                  ...
                                  .send({firstName: 'Buster', lastName: 'Dart', email: 'bd@example.com', roleId:2, active: true, password: "xxxxxxxx"});
  ...
  describe('DELETE /users/:id', () => {
    ...
    it('should return 200 (for admins)', async () => {
      ...
      let response = await agent.post('/users/')
                                .set("Authorization", `Bearer ${adminToken}`)
                                .send({firstName: 'Luddy', lastName: 'Muckers', email: 'lm@example.com', roleId:2, active: true, password: "xxxxxxxx"});
    ...
    it('should return 400 (for admins) if the id does not match a user in the database', async () => {
      const response = await agent.delete('/users/999').set("Authorization", `Bearer ${adminToken}`);
      expect(response.status).toBe(400);
    });
  });
