...
function getBlogList(folderPath) {
  ...
  blogFiles.forEach(fileName => {
    if (fileName.endsWith(".md")) {
      const blogInfo = getMarkdownMetaData(path.join(folderPath, fileName));
      if (blogInfo.published) {
        blogInfo.slug = fileName.replace(".md", ""); // ✅ Ensure slug is set correctly
        blogInfo.link = "/blog/" + blogInfo.slug;
